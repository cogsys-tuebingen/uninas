...
class CreamOfTheCropMethod(AbstractOptimizationMethod):
    ...
    def __init__(self, hparams: Namespace):
        ...
        # meta matching network and search network optimizers, remove the weight decay for the MMN
        ...
        self._optimizer_mmn.set_optimizer_lr(self._parsed_argument('mmn_lr', hparams, index=None))
    ...
    def _generic_step(self, batch, batch_idx, key: str, batch_augments: AbstractBatchAugmentation, **net_kwargs)\
        ...
        if key.startswith('train'):
            ...
            for estimator in self.estimators:
                ...
                log_dct['%s/estimators/%s' % (key, estimator.key)] = ResultValue(v, inputs.shape[0])
            ...
            if self.board.get_pareto_best().is_empty() or self.board.is_in_grace_time(self.current_epoch):
                ...
                log_dct.update({
                    ...
                    key + '/loss_student': ResultValue(loss_student, inputs.shape[0]),
                    key + '/loss_to_teacher': ResultValue(loss_to_teacher, inputs.shape[0])
                })
